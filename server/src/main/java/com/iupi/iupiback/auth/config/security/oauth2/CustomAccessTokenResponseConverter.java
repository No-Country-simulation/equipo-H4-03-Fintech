...
public class CustomAccessTokenResponseConverter implements Converter<Map<String, Object>, OAuth2AccessTokenResponse> {
    ...
    public OAuth2AccessTokenResponse convert(Map<String, Object> tokenResponseParameters) {
        log.info("Estoy en OAuth2AccessTokenResponse");
        ...
        // Crear una copia de los parámetros adicionales excluyendo los parámetros estándar
        Map<String, Object> additionalParameters = new HashMap<>();
        tokenResponseParameters.entrySet().stream()
                .filter(e -> !TOKEN_RESPONSE_PARAMETER_NAMES.contains(e.getKey()))
                .forEach(e -> additionalParameters.put(e.getKey(), e.getValue()));
        // Asegurarse de incluir el id_token si existe
        if (tokenResponseParameters.containsKey("id_token")) {
            additionalParameters.put("id_token", tokenResponseParameters.get("id_token"));
        }
